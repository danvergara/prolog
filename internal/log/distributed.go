...
// Append appends the record to the log. Unlike in Code the Store,
// where we append the record directly to this server's log, we tell
// Raft to apply to a command (we've reused for the ProduceRequest for the
// command) that tells the FSM to append the record to the log.
// Raft runs the process described in Log Replication,
// to replicate the command to a majority of the Raft servers
...
// Even though we have only one request type, the append request type,
// I've written things that easily support multiple request types to show you
// would set up your own services when you have different requests.
// In apply(), we marshal the request type and request into bytes that
// Raft uses as the record's data it replicates.
// The l.raft.Apply(buf.Bytes(), timeout) call has a lot going on behind the scenes,
// running the steps described in Log Replication, to replicate the record and append
// the record to the leader's log.
...
// When you're ok with relaxed consistency, read operations
// need not to go through Raft.
...
// NewStreamLayer returns a pointer to a StreamLayer instance.
...
// RaftRPC byte to identify the connection type.
const RaftRPC = 1

// Accept is the mirror of Dial. We accept the incoming connection
// and read the byte that identifies the connection and then create
...
// Close closes the listener listener.
...
// If we configure the stream layer with a peer TLS config, we make a TLS client-side
// connection.
...
// but Raft supports adding servers as non-voters with the AddNonVoter() API.
...
// Removing the lider will trigger a new election.
